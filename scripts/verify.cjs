...
(async()=>{
 ...
 await page.goto('http://127.0.0.1:4173/?level=4',{waitUntil:'networkidle'});await page.waitForFunction(()=>window.clubGilmore?.ready);
 ...
 const mobile=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});await mobile.goto('http://127.0.0.1:4173/?level=4');await mobile.waitForFunction(()=>window.clubGilmore?.ready);await mobile.locator('#browser-toggle').click();await mobile.getByRole('button',{name:'Main gym',exact:true}).click();await mobile.keyboard.press('Escape');assert.equal(await mobile.evaluate(()=>document.activeElement.id),'browser-toggle');
